...
func readFileList(input []byte) (fileList, error) {
	logger.Debug("Updater: Parsing signed file list (input size: %d bytes)", len(input))

	logger.Debug("Updater: Decoding public key from base64")
	...
	logger.Debug("Updater: Public key decoded successfully (length: %d)", len(publicKeyBytes))
	...
	logger.Debug("Updater: Manifest has 3 lines as expected")
	...
	logger.Debug("Updater: Untrusted comment found: %s", string(lines[0]))

	logger.Debug("Updater: Decoding signature from base64")
	...
	logger.Debug("Updater: Signature decoded (length: %d)", len(signatureBytes))

	if len(signatureBytes) != ed25519.SignatureSize+10 || !bytes.Equal(signatureBytes[:10], publicKeyBytes[:10]) {
		logger.Debug("Updater: Signature length/keyid mismatch - sigLen: %d, expected: %d", len(signatureBytes), ed25519.SignatureSize+10)
		return nil, errors.New("Signature input bytes are incorrect length, type, or keyid")
	}
	logger.Debug("Updater: Signature format valid, verifying...")
	...
	logger.Debug("Updater: ✓ Ed25519 signature verified successfully")

	fileLines := strings.Split(string(lines[2]), "\n")
	logger.Debug("Updater: Parsing %d file hash lines", len(fileLines))
	fileHashes := make(map[string]fileEntry, len(fileLines))
	for index, line := range fileLines {
		if len(line) == 0 && index == len(fileLines)-1 {
			logger.Debug("Updater: Skipping empty last line")
			break
		}
		...
		if downloadLocation != "" {
			logger.Debug("Updater: Parsed file entry: %s (hash: %x, location: %s)", filename, hash, downloadLocation)
		} else {
			logger.Debug("Updater: Parsed file entry: %s (hash: %x, location: <default>)", filename, hash)
		}
	}
	if len(fileHashes) == 0 {
	...
	logger.Debug("Updater: Successfully parsed %d file entries from manifest", len(fileHashes))
